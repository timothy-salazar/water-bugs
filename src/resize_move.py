...
# trout_df.set_index('file_name',inplace=True)
# bug_df.set_index('file_name',inplace=True)
temp_df = pd.concat([trout_df,bug_df])
temp_df = temp_df.drop_duplicates(subset='file_name',keep='last')
# temp_df = temp_df[~temp_df.index.duplicated(keep='last')]
df = df.drop_duplicates(subset='file_name',keep='last')
df = df.merge(temp_df,left_on='file_name',right_on='file_name')
...
with open('../data/resized_3/meta.txt','w') as f:
    for i in range(df.shape[0]):
        f.write(';'.join(df.iloc[i].values.astype('str'))+'\n')
